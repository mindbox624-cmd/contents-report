...
- `.env`에 다음 값이 모두 채워져 있어야 한다: `YOUTUBE_API_KEY`, `NOTION_TOKEN`,
  `NOTION_DATABASE_ID`, `GMAIL_ADDRESS`, `GMAIL_APP_PASSWORD`, `REPORT_RECIPIENT_EMAIL`
...
```
python tools/send_gmail.py --pdf .tmp/weekly_report_20260921.pdf --subject "9월 21일 주간 콘텐츠 트렌드 리포트"
```
...
- **노션 기록 실패 (속성 이름 불일치 등)**: 데이터베이스 속성 이름이 위 체크리스트와
  ...
  데이터베이스에 맞게 업데이트한다.
- **이메일 발송 실패**: 앱 비밀번호가 만료/폐기됐을 가능성이 크다. 새로 발급받아
  `.env`와 클라우드 루틴의 환경변수를 함께 갱신한다.
...
- 지정된 Gmail 주소로 PDF가 첨부된 메일이 도착했다
